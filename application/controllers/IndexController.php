...
function ukieriAction() { $this->view->setFile ("content", "ukieri.xml"); echo $this->view->render("layout"); }
//function ukierigenesisAction() { $this->view->setFile ("content", "ukierigenesis.xml"); echo $this->view->render("layout"); }
//function participantsAction() { $this->view->setFile ("content", "participants.xml"); echo $this->view->render("layout"); }
//function travelAction() { $this->view->setFile ("content", "travel.xml"); echo $this->view->render("layout"); }  
//function hostsAction() { $this->view->setFile ("content", "hosts.xml"); echo $this->view->render("layout"); }
//function contactusAction() { $this->view->setFile ("content", "contactus.xml"); echo $this->view->render("layout"); }
//function audienceAction() { $this->view->setFile ("content", "audience.xml"); echo $this->view->render("layout"); }
//function sponsorsAction() { $this->view->setFile ("content", "sponsors.xml"); echo $this->view->render("layout"); }
//function tradefairAction() { $this->view->setFile ("content", "tradefair.xml"); echo $this->view->render("layout"); }
//function hostAction() { $this->view->setFile ("content", "host.xml"); echo $this->view->render("layout"); }
  ...
  function addaccountAction()
  {
    ...
    $user  = new User();
$passwd = $this->createRandomPassword();
...
$mail = new Zend_Mail();
$mail->setBodyText(' ');
$mail->setBodyHtml('Welcome to Paper Submission service of UCC13.<br /><br /> Your login Information :<br /> Login: <b>'.$data["email"].'</b><br />Password: <b>'.$passwd.'</b><br />Login here : http://paper.ukiericoncretecongress.com/index/login <br /><br /> We Recommend you to change you password, as soon as possible, visit here : <br /> http://paper.ukiericoncretecongress.com/index/editaccount <br /><br /><br /><br /> Best Regards,<br />UCC Team.<br/>IN case of website related queries send mail to hsrai@gndec.ac.in' );
$mail->setFrom('office@ukiericoncretecongress.com', 'UCC12');
$mail->addTo($data["email"], '');
$mail->setSubject('Your account details at http://paper.ukiericoncretecongress.com');
$mail->send();
    }
    echo $this->view->render("layout");
  }
  ...
  function editaccountAction()
  {
	...
	$this->user->mobile=trim($this->user->mobile,"+");
$this->user->phone=trim($this->user->phone,"+");	
 $this->user->fax=trim($this->user->fax,"+");
//print_r($this);
...
$user = new User();
 $userRow = $user->findByEmail ($email);
          ...
          if (empty($email)) {
		$this->view->recall_msg="Email field empty.";
      //$this->_forward ("lostpassword", "index");
            $this->view->setFile ("content", "passwordrecallwrong.xml");

    }
    
	
    elseif(!preg_match("/^([\w\!\#$\%\&\'\*\+\-\/\=\?\^\`{\|\}\~]+\.)*[\w\!\#$\%\&\'\*\+\-\/\=\?\^\`{\|\}\~]+@((((([a-z0-9]{1}[a-z0-9\-]{0,62}[a-z0-9]{1})|[a-z])\.)+[a-z]{2,6})|(\d{1,3}\.){3}\d{1,3}(\:\d{1,5})?)$/i", $email)) {
	$this->view->recall_msg="Unvalid Email ID.";
	$this->view->setFile ("content", "passwordrecallwrong.xml");
	}
    elseif(!is_object($userRow)){
		$this->view->recall_msg="This email ID is not yet registered.";
	$this->view->setFile ("content", "passwordrecallwrong.xml");
	}
	else{
      // We must send the password to the user
      ...
      if (is_object($userRow)) {
        ...
        $userRow->password = md5($password);
        //$userRow->save();
$this->view->user_email=$userRow->email;
$this->view->assign_hash=md5($userRow->email);
//print_r($this);
        $userRow->putInView($this->view);
//print_r($this);
//$sname = "user_email";
  //  $this->$view->setVar($sname, $userRow->email);
//$tname = "assign_hash";
//$this->$view->setVar($tname, md5($userRow->email));
	// And, finally: send a message to the chair, and show a polite ack.
        $mail = new Mail (Mail::SOME_USER, $this->texts->mail->subj_password_recall,
        $this->view->getScriptPaths());
        $mail->setFormat(Mail::FORMAT_HTML);
        $mail->loadTemplate ($this->lang, "send_confirmation");
//send_password
        $mail->setTo($userRow->email);
        $mail->getEngine()->password = $password;
        $mail->send();
...
function passwordrecallverifiedAction(){
$email = $this->getRequest()->getParam('email');
$auth_code = $this->getRequest()->getParam('auth_code');
$user = new User();
 $userRow = $user->findByEmail ($email);
    if(md5($email)==$auth_code){
if (is_object($userRow)) {
        // Get the default password, and assign it to the user
