...
if(!in_array("city", $unchecked)){
  if ((preg_match("#[^a-zA-Z -\s]#", $this->city)) || ($this->city =="")) {
	...
	if (!in_array("mobile", $unchecked)) {
	$this->mobile = trim($this->mobile);
$this->mobile =trim($this->mobile,"+");
	if(!empty($this->mobile)){
      if (!preg_match('/^\d+$/', $this->mobile)) 
		{
		  $messages[] = $texts->author->mobile_error;}
		}
	}
	...
	if(!empty($this->phone)){
      if (!preg_match('/^\d+$/', $this->phone)) 
		{
		  $messages[] = $texts->author->phone_error;}
		}
	}
		if (!in_array("fax", $unchecked)) {
	$this->fax = trim($this->fax);
$this->fax=trim($this->fax,"+");
	if(!empty($this->fax)){
      if (!preg_match('/^\d+$/', $this->fax)) 
		{
		  $messages[] = $texts->author->fax_error;}
		}
	}
