...
while (list(, $valyu) = each($ext_array)) {

    

    if(strToLower($ext) == $valyu) 
	...
	//echo "asads";	
	 ...
	 $this->_messages[self::STORE_MESSAGE_ID] =
         ...
         "Invalid file format: (extension:$ext, required:)<br/>";
...
function fileExists ($requiredFile, $ext=0)

  {
    ...
    $paperName="";
    if(!$ext==0){
		$paperName = $this->filePath  ($requiredFile->id_phase, $requiredFile->file_code,$ext);	
		//echo $paperName;
	}
	else {
		$paperName = $this->filePath  ($requiredFile->id_phase, $requiredFile->file_code,
...
/*
  ...
  function download($requiredFile,$ext="")
 ...
 if($ext==""){
	...
	$file = $this->filePath  ($requiredFile->id_phase, $requiredFile->file_code, $ext);

       header("Content-disposition: attachment; filename=" . $requiredFile->file_code . $this->id . "." . $ext);
}
    header("Content-Type: application/force-download");
    header("Content-Transfer-Encoding: $type\n");
    header("Content-Length: ".filesize($file));
